...
from sqlalchemy import func, Column, Integer, DateTime, ForeignKey, String, Text, Float, Numeric, Enum, Boolean
...
class Car(base):
    ...
    def get_cars_select(state="active"):
        if state == "all":
            ...
            return Car.query.filter(Car.status == 1).order_by(Car.make_id.asc()).all()
        elif state == "pending":
